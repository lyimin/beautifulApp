    ...
    */
    func footerViewPullToRefresh(direction : XMRefreashDirection, callback : (()->Void)?) {
        ...
        if direction == .XMRefreshDirectionHorizontal {
            footView = XMRefreshFooterView(frame: CGRect(x: 0, y: 0, width: XMRefreshViewHeight, height: UIConstant.SCREEN_HEIGHT))
        } else {
            footView = XMRefreshFooterView(frame: CGRect(x: 0, y: 0, width: UIConstant.SCREEN_WIDTH, height: XMRefreshViewHeight))
        }
