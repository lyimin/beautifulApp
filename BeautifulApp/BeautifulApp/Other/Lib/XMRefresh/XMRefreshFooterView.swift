    ...
    override var State : XMRefreshState {
        ...
        didSet {
            ...
            case .RefreshStateNormal:
                ...
                if (.RefreshStateRefreshing == oldState && deltaH > 0  && currentCount != self.lastRefreshCount) {
                    if self.viewDirection == XMRefreashDirection.XMRefreshDirectionHorizontal {
                        ...
                        offset.x = self.scrollView.contentOffset.x - self.width + UIConstant.SCREEN_WIDTH
            ...
            case .RefreshStateRefreshing:
                if self.viewDirection == XMRefreashDirection.XMRefreshDirectionHorizontal {
                    self.scrollView.setContentOffset(CGPoint(x: self.scrollView.contentSize.width-UIConstant.SCREEN_WIDTH+self.width, y: 0), animated: true)
                } else {
